    ...
    public class FakeSender : IDisposable
    {
        ...
        public void Send()
        {
            ...
            enumerator.BeginExecute(SendInternal(enumerator), result =>
            {
                ...
                {
                    _logger.Error("Failed to send message", exception);
                    Failure(exception);
                }
            });
        }
            ...
            try
            {
                using (var client = new TcpClient())
                {
                    ...
                    using (var stream = client.GetStream())
                    {
                        ...
                        {
                            _logger.Info("Could not write to {0} because {1}", Destination,
                                              exception);
                            Failure(exception);
                            yield break;
                        }
                        ...
                        _logger.Info("Writing {0} bytes to {1}", buffer.Length, Destination);
                        ...
                        {
                            _logger.Info("Could not read confirmation from {0} because {1}", Destination,
                                              exception);
                            Failure(exception);
                            yield break;
                        }
