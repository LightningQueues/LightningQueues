...
public static class ReactiveExtensions
{
    public static async Task<T> FirstAsyncWithTimeout<T>(this IObservable<T> stream, TimeSpan timeSpan)
    {
        ...
        using (Observable.Interval(timeSpan).Subscribe(_ => completionSource.SetException(new TimeoutException())))
        {
    ...
    {
        return stream.FirstAsyncWithTimeout(TimeSpan.FromSeconds(10));
    }

    public static IObservable<int> RunningCount<T>(this IObservable<T> stream)
    {
        return stream.Scan(0, (acc, _) => acc + 1);
    }
}